...
function play(track) {
    ...
    if (listenButtonIcon) listenButtonIcon.replaceChildren(loadingIcon.cloneNode(true));
    if (listenButtonLabel) listenButtonLabel.textContent = PLAYER_JS_T.pause;
...
function seek(track, onComplete = null) {
    ...
    seeking.cancel = () => {
        ...
        if (listenButtonIcon) listenButtonIcon.replaceChildren(playIcon.cloneNode(true));
        if (listenButtonLabel) listenButtonLabel.textContent = PLAYER_JS_T.listen;
    };
...
for (const container of document.querySelectorAll('.track')) {
    ...
    audio.addEventListener('play', event => {
        ...
        if (listenButtonIcon) listenButtonIcon.replaceChildren(pauseIcon.cloneNode(true));
        if (listenButtonLabel) listenButtonLabel.textContent = PLAYER_JS_T.pause;
